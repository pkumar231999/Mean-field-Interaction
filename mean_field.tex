...
goes around the whole inner part of the ring. The two paths are recombine at QPC B and continue to either contact 1 (which we call the transmitted path) or to contact $2$ (which we call the reflected path). We call $d_{\rm A}$ (resp. $D_{\rm A}=|d_{\rm A}|^2$) the transmission amplitude (resp. probability) of QPC A
...
and therefore,
\begin{equation}
    n_1 =  D_{\rm tot} \int_{-\infty}^{\infty} dt \frac{eV_0(t)}{h}.
\end{equation}
...
\begin{equation}
\rho_i(t) \equiv  \langle c_{i \bar \sigma}^{\dagger}(t)c_{i\bar \sigma}(t)\rangle.
\end{equation}
...
\pk{For our Gaussian voltage pulse, the acquired phase $\phi(t)$ is from Eq.(\ref{eq:pulse_phase}), where $V_{o}$ is defined using Eq.(\ref{eq:gauss_pulse}) gives,}
...
$17050$ lattice sites for the full Mach-Zehnder.
...
(c) Same as (b) but versus energy for the lowest Landau level $v_0(E)$. \pk{The left and the right red bars are respectively the bottom of the lowest and first Landau levels.}}
\end{figure}
...
(b) Transmission and reflection probability versus magnetic field $\phi_a$ at $E=-3.2$. \pk{Inset: Colormap of the local current density injected from contact $0$ at $E=-3.2$ and $\phi_a=0.44$. One clearly observes that the propagation takes place on the right edge.}} 
\end{figure}
